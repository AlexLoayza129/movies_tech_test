<template>
    <div>
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-3xl text-center text-red-600 mb-2">Turnos</h3>
            <a :href="createShift" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700">Agregar un turno</a>
        </div>
        <table class="bg-white rounded-md">
            ...
            <tbody>
                <!-- Movie Counter -->
                <span class="hidden">{{ shiftCounter = 1 }}</span>
                <!-- Movies -->
                <tr v-for="(shift, index) in listOfShifts" :key="index" class="hover:bg-gray-300">
                    <td v-if="shift.status == 1" class="p-4 text-center">{{ shiftCounter++ }}</td>
                    <td v-if="shift.status == 1" class="p-4 text-center">{{ shift.id }}</td>
                    <td v-if="shift.status == 1" class="p-4 text-center">{{ formatDate(shift.time) }}</td>
                    <td v-if="shift.status == 1" class="p-4 text-center">{{ shift.movie_name }}</td>
                    <td v-if="shift.status == 1" class="p-4 text-center">
                        <span v-if="shift.active == 1" class="text-green-500 font-bold">
                            Activo
                        </span>
                    ...
                    <td v-if="shift.status == 1" class="p-4 flex justify-center items-center">
                        <!-- Is Active? -->
                        <form :action="changeStatus" method="POST">
                            <csrf-component></csrf-component>
                            <div v-if="shift.active == 1">
                    ...
                    <td v-if="shift.status == 1" class="p-4">
...
export default {
    ...
    data(instance){
        return{
            createShift: route('admin.shifts.create'),
            listOfShifts: [],
            changeStatus: route('admin.shifts.change'),
            deleteShift: route('admin.shifts.destroy'),
            editShift: route('admin.shifts.edit'),
        }
    },
    methods:{
        formatDate(value) {
    ...
    mounted () {
        axios
            .get('http://localhost:8000/api/shifts')
                .then(response => (this.listOfShifts = response.data))
    }
}
</script>